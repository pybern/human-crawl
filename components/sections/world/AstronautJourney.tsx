...
/**
 * Scroll-driven cinematic: the camera (with the astronaut floating just ahead)
 * flies down the -Z axis through three worlds — open space, a glowing wireframe
 * grid tunnel, and a crystalline cave — while the fog colour shifts. Progress
 * (0..1) comes from `cinema.progress`, set by the section's ScrollTrigger.
 */
const SPACE_END = -34;
const GRID_END = -80;
...
export default function AstronautJourney({ mobile = false }: { mobile?: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    if (astro.current) {
      ...
      astro.current.rotation.x = Math.cos(t * 0.3) * 0.15;
    }
...
function GridTunnel({ mobile }: { mobile: boolean }) {
  ...
  const ringGeo = useMemo(() => {
    // square outline (closed by lineLoop)
    const s = 9;
    const pts = [
  ...
  const spacing = (GRID_END - SPACE_END) / rings; // negative step
  return (
    <group ref={group}>
